...
import (
	...
	xtreamapi "github.com/xybydy/iptv-proxy/pkg/xtream-proxy"
)
...
func (c *Config) cacheXtreamM3u(m3uURL fmt.Stringer) error {
...
func (c *Config) xtreamPanelAPIGET(ctx *gin.Context) {
	...
	resp, httpcode, err := client.Action(c.ProxyConfig, "get_panel", nil)
	if err != nil {
...
func (c *Config) xtreamHlsStream(ctx *gin.Context) {
	...
	reqUrl, err := getHlsRedirectURL(channel)
	if err != nil {
		ctx.AbortWithError(http.StatusInternalServerError, err) // nolint: errcheck
		return
	}

	req, err := reqUrl.Parse(
		fmt.Sprintf(
			"%s://%s/hls/%s/%s",
			reqUrl.Scheme,
			reqUrl.Host,
...
func getHlsRedirectURL(channel string) (*url.URL, error) {
	...
	reqUrl, ok := hlsChannelsRedirectURL[channel+".m3u8"]
	if !ok {
		return nil, errors.New("HSL redirect reqUrl not found")
	}
