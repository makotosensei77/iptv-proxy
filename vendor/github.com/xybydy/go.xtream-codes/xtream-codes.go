...
func (c *XtreamClient) GetPanel() (*PanelInfo, error) {
	data, err := c.sendRequest("panel_api.php", nil)
	if err != nil {
		return nil, err
	}

	panel := new(PanelInfo)
	jsonErr := json.Unmarshal(data, &panel)

	return panel, jsonErr
}
