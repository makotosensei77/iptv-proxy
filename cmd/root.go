...
var rootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if xtreamUser != "" && xtreamBaseURL != "" && xtreamPassword != "" && remoteHostURL.String() == "" {
			remoteHostURL, err = url.Parse(fmt.Sprintf("%s/get.php?username=%s&password=%s&type=m3u_plus&output=ts", xtreamBaseURL, xtreamUser, xtreamPassword))
			if err != nil {
				log.Fatal(err)
			}
		}
		...
		if _, err := os.Stat(conf.M3UFileName); os.IsNotExist(err) && (xtreamBaseURL == "" || xtreamPassword == "" || xtreamUser == "") && m3uURL == "" {
			log.Fatal("no input provided. please provide url or filepath or xtream inputs")
		}
...
func init() {
	...
	rootCmd.Flags().String("hostname", "0.0.0.0", "Hostname or IP to expose the IPTVs endpoints")
